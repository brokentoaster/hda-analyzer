...
CTL_ELEM_IFACEs = {"card": 0, "mixer": 2, "pcm": 3}
CTL_ELEM_RIFACEs = {}
...
class AlsaMixerElem:
    ...
    def __info(self):
        ...
        if b["type"] == CTL_ELEM_TYPE_INTEGER:
            b["min"], b["max"], b["step"] = struct.unpack("lll", a[4][: LONGSIZE * 3])
        elif b["type"] == CTL_ELEM_TYPE_INTEGER64:
            b["min"], b["max"], b["step"] = struct.unpack(
                "qqq", a[4][: LONGLONGSIZE * 3]
            )
        elif b["type"] == CTL_ELEM_TYPE_ENUMERATED:
            b["items"], b["item"], b["name"] = struct.unpack(
                "II64s", a[4][: UINTSIZE * 2 + 64]
            )
        ...
        if self.type == CTL_ELEM_TYPE_BOOLEAN:
            return [
                ...
                for x in struct.unpack(
                    "l" * self.count, res[startoff : startoff + self.count * LONGSIZE]
                )
            ]
            ...
            return struct.unpack(
                "q" * self.count, res[startoff : startoff + self.count * LONGLONGSIZE]
            )
        elif self.type == CTL_ELEM_TYPE_ENUMERATED:
            return struct.unpack(
                "I" * self.count, res[startoff : startoff + self.count * UINTSIZE]
            )

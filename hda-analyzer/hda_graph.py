...
class Node:
    ...
    def expose(self, cr, event, graph):
        ...
        cr.select_font_face(
            "Misc Fixed", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL
        )
        cr.set_font_size(14)
        ...
        cr.rectangle(
            self.myarea[0], self.myarea[1] + (height / 4) * 3, width / 2, height / 4
        )
        cr.rectangle(
            self.myarea[0] + width / 2,
            self.myarea[1] + (height / 4) * 3,
            width / 2,
            height / 4,
        )
        cr.stroke()
        ...
        cr.stroke()
        cr.move_to(
            self.myarea[0] + width / 2 + 20, self.myarea[1] + (height / 4) * 3 + 15
        )
        cr.text_path("OUT")
        cr.stroke()
    ...
    def in_area(self, x, y):
        if (
            x >= self.myarea[0]
            and y >= self.myarea[1]
            and x < self.myarea[0] + self.myarea[2]
            and y < self.myarea[1] + self.myarea[3]
        ):
...
class Route:
    ...
    def longdesc(self):
        src = self.src.node
        dst = self.dst.node
        return "%s 0x%02x -> %s 0x%02x" % (
            src.wtype_id.replace("_", "-"),
            src.nid,
            dst.wtype_id.replace("_", "-"),
            dst.nid,
        )
    ...
    def statusdesc(self):
        ...
        res = (
            niceprint("SRC " + src, vals[0]) + " -> " + niceprint("DST " + dst, vals[1])
        )
        return res
    ...
    def expose(self, cr, event):
        ...
        if 0:  # direct green lines
            ...
            cr.move_to(
                self.src.myarea[0] + (width / 4) * 3, self.src.myarea[1] + height
            )
        ...
        for p in possible:
            ...
            for route in routes:
                ...
                for line in route.lines:
                    if (
                        check_dot(line[0], line[1], p)
                        or check_dot(line[2], line[3], p)
                        or check_dot(p[0], p[1], line)
                        or check_dot(p[2], p[3], line)
                    ):
                        # print "Found1", p
                        found = True
                        break
    ...
    def analyze_routes(self, routes, nodes):
        ...
        if posx >= dposx:
            ...
            for i in range(2, 10):
                a = list(
                    range(width * i + extra * i + 5, width * i + extra * (i + 1) - 1, 5)
                )
                a.reverse()
                xrange = xrange + a
        else:
        ...
        for j in xrange:
            possible.append(
                [startx + j, posy + height + 5, startx + j, dposy + height + 5]
            )
    ...
    def finish(self, routes, nodes):
        ...
        if tryit >= 0:
            ...
            print(
                "[1] displaced route 0x%x->0x%x %s %s"
                % (
                    self.src.node.nid,
                    self.dst.node.nid,
                    repr(self.lines[-1]),
        ...
        if tryit >= 0:
            ...
            print(
                "[2] displaced route 0x%x->0x%x %s %s"
                % (
                    self.src.node.nid,
                    self.dst.node.nid,
                    repr(self.lines[-1]),
...
class CodecGraphLayout(Gtk.Layout):
    def __init__(self, adj1, adj2, codec, mytitle, statusbar):
        GObject.GObject.__init__(self, adj1, adj2)
        self.set_events(0)
        self.add_events(
        ...
        self.changed_handler = HDA_SIGNAL.connect(
            "hda-node-changed", self.hda_node_changed
        )
    ...
    def __build(self, extra=50):
        ...
        for nid in self.codec.nodes:
            ...
            conns = max(self.codec.connections(nid, 0), self.codec.connections(nid, 1))
            if conns > maxconns:
                maxconns = conns
...
class CodecGraph(Gtk.Window):
    def __init__(self, codec):
        ...
        table.attach(
            self.layout, 0, 1, 0, 1, Gtk.AttachOptions.FILL | Gtk.AttachOptions.EXPAND, Gtk.AttachOptions.FILL | Gtk.AttachOptions.EXPAND, 0, 0
        )
        vScrollbar = Gtk.VScrollbar(None)
        table.attach(
            vScrollbar, 1, 2, 0, 1, Gtk.AttachOptions.FILL | Gtk.AttachOptions.SHRINK, Gtk.AttachOptions.FILL | Gtk.AttachOptions.SHRINK, 0, 0
        )
        hScrollbar = Gtk.HScrollbar(None)
        table.attach(
            hScrollbar, 0, 1, 1, 2, Gtk.AttachOptions.FILL | Gtk.AttachOptions.SHRINK, Gtk.AttachOptions.FILL | Gtk.AttachOptions.SHRINK, 0, 0
        )
        ...
        table.attach(
            statusbar, 0, 2, 2, 3, Gtk.AttachOptions.FILL | Gtk.AttachOptions.SHRINK, Gtk.AttachOptions.FILL | Gtk.AttachOptions.SHRINK, 0, 0
        )
        self.show_all()

...
(HDA_INPUT, HDA_OUTPUT) = list(range(2))
...
GPIO_IDS = {
    ...
    "unsol": (
        VERBS["GET_GPIO_UNSOLICITED_RSP_MASK"],
        VERBS["SET_GPIO_UNSOLICITED_RSP_MASK"],
    ),
    "sticky": (VERBS["GET_GPIO_STICKY_MASK"], VERBS["SET_GPIO_STICKY_MASK"]),
    "data": (VERBS["GET_GPIO_DATA"], VERBS["SET_GPIO_DATA"]),
}

EAPDBTL_BITS = {"BALANCED": 0, "EAPD": 1, "R/L": 2}

PIN_WIDGET_CONTROL_BITS = {"IN": 5, "OUT": 6, "HP": 7}

PIN_WIDGET_CONTROL_VREF = ["HIZ", "50", "GRD", None, "80", "100", None, None]
...
class HDAAmpCaps:
    ...
    def reread(self):
        caps = self.codec.param_read(
            self.nid, PARAMS[self.dir == HDA_OUTPUT and "AMP_OUT_CAP" or "AMP_IN_CAP"]
        )
        if caps == ~0 or caps == 0:
...
class HDAAmpVal:
    ...
    def get_val_str(self, idx):
        ...
        if self.stereo:
            return (
                "["
                + niceval(self.vals[idx * 2])
                + " "
                + niceval(self.vals[idx * 2 + 1])
                + "]"
            )
...
class HDANode:
    ...
    def reread_pwr(self, value=None):
        ...
        self.pwr_setting_name = (
            self.pwr_setting < 4 and POWER_STATES[self.pwr_setting] or "UNKNOWN"
        )
        self.pwr_actual_name = (
            self.pwr_actual < 4 and POWER_STATES[self.pwr_actual] or "UNKNOWN"
        )
        ...
        if dst_node.in_amp:
            ...
            if dst_node.amp_vals_in.indices == dst_node.connections:
                if not self.nid in dst_node.connections:
                    raise ValueError(
                        "nid 0x%02x is not connected to nid 0x%02x (%s, %s)"
                        % (
                            dst_node.nid,
                            self.nid,
                            repr(self.connections),
                            repr(dst_node.connections),
                        )
                    )
...
class HDACodec:
    ...
    def rw(self, nid, verb, param):
        ...
        if not self.exporter:
            ...
            return self.exporter.rw(
                self.exporta and self.exporta[-1] or False, nid, verb, param
            )
    ...
    def analyze(self):
        ...
        if self.subsystem_id == 0:
            self.subsystem_id = self.rw(
                self.afg and self.afg or self.mfg, VERBS["GET_SUBSYSTEM_ID"], 0
            )
    ...
    def dump(self, skip_nodes=False):
        ...
        def print_gpio(node):
            ...
            str = "GPIO: io=%d, o=%d, i=%d, unsolicited=%d, wake=%d\n" % (
                ...
                node.gpio_o,
                node.gpio_i,
                node.gpio_unsol and 1 or 0,
                node.gpio_wake and 1 or 0,
            )
            for i in range(node.gpio_max):
                str += (
                    "  IO[%d]: enable=%d, dir=%d, wake=%d, sticky=%d, "
                    "data=%d, unsol=%d\n"
                    % (
                        i,
    ...
    def dump_node(self, node):
        ...
        def print_power_state(node):
            ...
            return "  Power: setting=%s, actual=%s\n" % (
                node.pwr_setting_name,
                node.pwr_actual_name,
            )
        ...
        def doplace(nid, y, x):
            ...
            if node.wtype_id == "AUD_MIX":
                ...
                while 1:
                    ...
                    if (
                        res[y][x + 1] is None
                        and res[y][x - 1] is None
                        and res[y + 1][x] is None
                        and res[y - 1][x] is None
                        and res[y][x] is None
                    ):
                        res[y][x] = nid
...
os.close(FD)
""" % (
                "/dev/snd/hwC%sD%s" % (codec.card, codec.device),
                text,
            )
        return text
...
if __name__ == "__main__":
    ...
    v.analyze()
    print(
        "vendor_id = 0x%x, subsystem_id = 0x%x, revision_id = 0x%x"
        % (v.vendor_id, v.subsystem_id, v.revision_id)
    )
    print(
        "afg = %s, mfg = %s"
        % (v.afg and "0x%x" % v.afg or "None", v.mfg and "0x%x" % v.mfg or "None")
    )
    print()
    print()
    print(v.dump()[:-1])

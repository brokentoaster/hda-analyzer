...
class HDApcmControl:
    ...
    def dump_extra(self):
        str = '  Control: %sname="%s", index=%s, device=%s\n' % (
            self.iface and ('iface="%s", ' % self.iface) or "",
            self.name,
            self.index,
            self.device,
        )
...
class ProcNode(HDABaseProc):
    ...
    def rw(self, verb, param):
        if verb in self.verbs:
            ...
            if val >= len(self.amp_vals[dir]):
                print(
                    "AMP index out of range (%s >= %s)" % (val, len(self.amp_vals[dir]))
                )
            ...
            return param
        raise ValueError(
            "unimplemented node rw (0x%x, 0x%x, 0x%x)" % (self.nid, verb, param)
        )
    ...
    def param_read(self, param):
        ...
        raise ValueError(
            "unimplemented node param read (0x%x, 0x%x)" % (self.nid, param)
        )
    ...
    def add_converter(self, line):
        ...
        self.add_verb(VERBS["GET_CONV"], ((stream & 0x0F) << 4) | (channel & 0x0F))
    ...
    def add_pcm(self, line1, line2, line3):
        ...
        self.add_param(PARAMS["PCM"], (tmp1 & 0xFFFF) | ((tmp2 & 0xFFFF) << 16))
    ...
    def add_unsolicited(self, line):
        ...
        self.add_verb(
            VERBS["GET_UNSOLICITED_RESPONSE"], (tag & 0x3F) | ((enabled & 1) << 7)
        )
    ...
    def add_power(self, line):
        ...
        self.add_verb(
            VERBS["GET_POWER_STATE"], (setting & 0x0F) | ((actual & 0x0F) << 4)
        )
    ...
    def add_processcaps(self, line):
        ...
        self.add_param(PARAMS["PROC_CAP"], (benign & 1) | ((ncoeff & 0xFF) << 8))
    ...
    def add_volknob(self, line):
        ...
        self.add_verb(
            VERBS["GET_VOLUME_KNOB_CONTROL"], ((direct & 1) << 7) | (val & 0x7F)
        )
    ...
    def parse(self, str):
        ...
        def decodeampcap(idx, prefix):
            if lines[idx].startswith(prefix):
                ...
                return idx + 1, (ofs & 0x7F) | ((nsteps & 0x7F) << 8) | (
                    (stepsize & 0x7F) << 16
                ) | ((mute & 1) << 31)
        ...
        def decodegpiocap(idx, prefix):
            if lines[idx].startswith(prefix):
                ...
                return idx + 1, (io & 0xFF) | ((o & 0xFF) << 8) | ((i & 0xFF) << 16) | (
                    (unsol & 1) << 30
                ) | ((wake & 1) << 31)
        ...
        if line == "Invalid AFG subtree":
            ...
            return
        while line.startswith("Power-Map: ") or line.startswith("Analog Loopback: "):
